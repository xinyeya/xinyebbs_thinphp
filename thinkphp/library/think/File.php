...
class File extends SplFileObject
{
    ...
    public function checkExt($ext)
    {
        ...
        if (!in_array($extension, $ext)) {
            $this->error = 'extensions to uploads is not allowed';
            return false;
        }
    ...
    public function checkMime($mime)
    {
        ...
        if (!in_array(strtolower($this->getMime()), $mime)) {
            $this->error = 'mimetype to uploads is not allowed';
            return false;
        }
    ...
    public function move($path, $savename = true, $replace = true, $autoAppendExt = true)
    {
        ...
        // 检测合法性
        if (!$this->isValid()) {
            $this->error = 'uploads illegal files';
            return false;
        }
        ...
        if ($this->isTest) {
            ...
            $this->error = 'uploads write error';
            return false;
        }
    ...
    private function error($errorNo)
    {
        switch ($errorNo) {
            case 1:
            case 2:
                $this->error = 'uploads File size exceeds the maximum value';
                break;
            case 3:
            ...
            case 4:
                ...
                break;
            case 6:
                $this->error = 'uploads temp dir not found';
                break;
            case 7:
                $this->error = 'file write error';
                break;
            default:
                $this->error = 'unknown uploads error';
        }
    }
